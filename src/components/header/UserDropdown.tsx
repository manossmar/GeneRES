...
export default function UserDropdown() {
  ...
  const fullName = user ? `${formatName(user.first_name)} ${formatName(user.last_name)}` : 'User';
  const email = user?.email?.toLowerCase() || '';

  const displayName = user
    ? `${formatName(user.first_name)} ${user.last_name ? user.last_name.charAt(0).toUpperCase() + '.' : ''}`
    : 'User';
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11 flex items-center justify-center bg-gray-200 dark:bg-gray-700">
          {profilePictureUrl && !imgError ? (
            ...
            <span className="text-gray-500 dark:text-gray-400 font-medium text-lg">{userInitials}</span>
          )}
        </span>
